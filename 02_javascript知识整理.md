...
在JavaScript中，`forEach`、`map`、`filter`、`reduce`等数组遍历方法在处理数组时，如果修改了当前索引后面的内容，这通常不会影响正在进行的遍历过程，因为这些都是按照数组元素的固定索引进行遍历的。
然而，如果你修改了数组的长度，比如通过`pop`、`shift`、`splice`等方法移除元素，或者通过`push`、`unshift`、`splice`等方法添加新元素，那么正在进行的遍历行为可能会受到影响。因为遍历是基于数组长度进行的，如果长度发生变化，遍历的次数和元素可能会与预期不同。
例如，使用`forEach`时修改数组：
```javascript
let arr = [1, 2, 3, 4, 5];
arr.forEach((item, index) => {
  if (index === 0) {
    arr.push(6); // 在遍历开始时添加一个新元素
  }
  console.log(item); // 依次输出：1, 2, 3, 4, 5
  // 注意，尽管添加了新元素，但forEach仍然会按照原始长度遍历数组
});
```
在上面的例子中，尽管在遍历开始时向数组添加了一个新元素，但`forEach`仍然按照原始长度遍历数组，新添加的元素`6`不会在本次遍历中被访问到。
如果你在遍历过程中修改了数组元素本身，这通常不会影响遍历过程，但会影响到后续的访问，因为数组元素已经被修改：
```javascript
let arr1 = [1, 2, 3, 4, 5];
arr1.forEach((item, index) => {
  arr1[index + 1] = item + 10; // 修改后续元素
  console.log(item); // 依次输出：1, 11, 21, 31, 41
  // 注意，后续元素已经被修改
});
```
在这个例子中，每次遍历都会修改后续元素的值，因此打印出的值会随着遍历的进行而改变。
总之，当你在遍历数组时修改数组，通常建议避免这样做，除非你明确知道自己在做什么，因为这可能会导致不可预测的行为。如果你需要在遍历过程中创建一个新数组，可以考虑使用`map`方法，或者使用`slice`或`concat`来创建一个原始数组的副本，然后在副本上进行操作。
...
- 在`JavaScript`中，当我们对一个对象的属性进行赋值操作时，如果这个属性之前不存在于对象上，`JavaScript`会在这个对象上创建这个属性并赋值。因此方式1会直接修改原对象。
- 而方式2中是可以理解为对**对象属性的属性**进行了赋值，因此首先要找到对象的属性`arr`以及`person`，所以沿着原型链找到`obj`，其身上有对应属性，然后再对`arr`和`person`的属性进行赋值，最终导致了原型`obj`发生了变化，而对象本身没有发生变化。
